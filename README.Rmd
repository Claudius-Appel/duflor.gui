...
In order for the shiny application to be able to run the analysis in parallel, the packages `duflor` and `duflor.gui` **must** be installed. If this package is developed locally, executing `devtools::load_all()` will not be sufficient. To counteract this, make sure that both packages are actually installed as such, and show up in the listed packages.
